jQuery(document).ready(function($) {
	...
	if ( typeof propertySubmit !== "undefined" ) {		var ajaxURL = propertySubmit.ajaxURL;        var uploadNonce = propertySubmit.uploadNonce;        var fileTypeTitle = propertySubmit.fileTypeTitle;        /* Apply jquery ui sortable on gallery items */        $( "#floor-plan-thumbs-container" ).sortable({            revert: 100,            placeholder: "sortable-placeholder",            cursor: "move"        });				/* initialize uploader */		        var flooruploader = new plupload.Uploader({            browse_button: 'floor-plan-select-images',            file_data_name: 'inspiry_upload_file',            container: 'floor-plan-plupload-container',            drop_element: 'floor-plan-drag-and-drop',            url: ajaxURL + "?action=ajax_img_upload&nonce=" + uploadNonce,            filters: {                mime_types : [                    { title : fileTypeTitle, extensions : "jpg,jpeg,gif,png" }                ],                max_file_size: '10000kb',                prevent_duplicates: true            }        });        flooruploader.init();				/* Run after adding file */        flooruploader.bind('FilesAdded', function(up, files) {            var html = '';            var galleryThumb = "";            plupload.each(files, function(file) {                galleryThumb += '<div id="holder-' + file.id + '" class="gallery-thumb">' + '' + '</div>';            });            document.getElementById('floor-plan-thumbs-container').innerHTML += galleryThumb;            up.refresh();            flooruploader.start();        });						/* Run during upload */        flooruploader.bind('UploadProgress', function(up, file) {            document.getElementById( "holder-" + file.id ).innerHTML = '<span>' + file.percent + "%</span>";        });        /* In case of error */        flooruploader.bind('Error', function( up, err ) {            document.getElementById('floor-plan-errors-log').innerHTML += "<br/>" + "Error #" + err.code + ": " + err.message;        });        /* If files are uploaded successfully */        flooruploader.bind('FileUploaded', function ( up, file, ajax_response ) {            var response = $.parseJSON( ajax_response.response );            if ( response.success ) {                var galleryThumbHtml = '<img src="' + response.url + '" alt="" />' +                '<a class="remove-image" data-property-id="' + 0 + '"  data-attachment-id="' + response.attachment_id + '" href="#remove-image" ><i class="fa fa-trash-o"></i></a>' +                '<a class="mark-featured" data-property-id="' + 0 + '"  data-attachment-id="' + response.attachment_id + '" href="#mark-featured" ><i class="fa fa-star-o"></i></a>' +                '<input type="hidden" class="gallery-image-id" name="floor_image_ids[]" value="' + response.attachment_id + '"/>' +                '<span class="loader"><i class="fa fa-spinner fa-spin"></i></span>';                document.getElementById( "holder-" + file.id ).innerHTML = galleryThumbHtml;                floorbindThumbnailEvents();             } else {                                console.log ( response );            }        });						/* Bind thumbnails events with newly added gallery thumbs */        var floorbindThumbnailEvents = function () {			$('a.remove-image').unbind('click');            $('a.mark-featured').unbind('click');            			$('a.mark-featured').click(function(event){                event.preventDefault();                var $this = $( this );                var starIcon = $this.find( 'i');                if ( starIcon.hasClass( 'fa-star-o' ) ) {                       $('.gallery-thumb .featured-img-id').remove();                     $('.gallery-thumb .mark-featured i').removeClass( 'fa-star').addClass( 'fa-star-o' );                      var $this = $( this );                    var input = $this.siblings( '.gallery-image-id' );                      var featured_input = input.clone().removeClass( 'gallery-image-id' ).addClass( 'featured-img-id' ).attr( 'name', 'featured_image_id' );                      $this.closest( '.gallery-thumb' ).append( featured_input );                     starIcon.removeClass( 'fa-star-o' ).addClass( 'fa-star' );                 }            });												$('a.remove-image').click(function(event){                event.preventDefault();                var $this = $(this);                var gallery_thumb = $this.closest('.gallery-thumb');                var loader = $this.siblings('.loader');                loader.show();                var removal_request = $.ajax({                    url: ajaxURL,                    type: "POST",                    data: {                        property_id : $this.data('property-id'),                        attachment_id : $this.data('attachment-id'),                        action : "remove_gallery_image",                        nonce : uploadNonce                    },                    dataType: "html"                });                removal_request.done(function( response ) {                    var result = $.parseJSON( response );                    if( result.attachment_removed ){                        gallery_thumb.remove();                    } else {                        document.getElementById('floor-plan-errors-log').innerHTML += "<br/>" + "Error : Failed to remove attachment";                    }                });                removal_request.fail(function( jqXHR, textStatus ) {                    alert( "Request failed: " + textStatus );                });            });  									        };          floorbindThumbnailEvents(); 				    }   	
});