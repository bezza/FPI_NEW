...
if( isset( $_POST['action'] ) && is_user_logged_in() ) {
    ...
    if( wp_verify_nonce( $_POST['property_nonce'], 'submit_property' ) ){
            ...
            if( $property_id > 0 ){
                ...
                if( $action == "update_property" ){
                    delete_post_meta( $property_id, 'REAL_HOMES_property_images' );                    					delete_post_meta( $property_id, 'REAL_HOMES_floor_plan' );
				...
				// Attach floor images with newly created property                if ( isset( $_POST['floor_image_ids'] ) ) {                    if( ! empty ( $_POST['floor_image_ids'] ) && is_array ( $_POST['floor_image_ids'] ) ) {                        $floor_image_ids = array();                        foreach ( $_POST['floor_image_ids'] as $floor_image_ids ) {                            $floor_image_ids[] = intval( $floor_image_ids );                            add_post_meta( $property_id, 'REAL_HOMES_floor_plan', $floor_image_ids );                        }                        if ( isset( $_POST['featured_image_id'] ) ) {                            $featured_image_id = intval( $_POST['featured_image_id'] );                            if ( in_array( $featured_image_id, $floor_image_ids ) ) {     // validate featured image id                                update_post_meta ( $property_id, '_thumbnail_id', $featured_image_id );                                /* if video url is provided but there is no video image then use featured image as video image */                                if ( empty( $tour_video_image ) && !empty( $_POST['video-url'] ) ) {                                    update_post_meta( $property_id, 'REAL_HOMES_tour_video_image', $featured_image_id );                                }                            }                        } else if( ! empty ( $floor_image_ids ) ) {                            update_post_meta ( $property_id, '_thumbnail_id', $floor_image_ids[0] );                        }                    }                }								
