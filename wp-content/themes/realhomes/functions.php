...
/*-----------------------------------------------------------------------------------*/
	...
	require_once(get_template_directory() . '/framework/functions/property_filter.php');
	require_once(get_template_directory() . '/property-details/property-market-information.php');
...
/*-----------------------------------------------------------------------------------*/
    ...
    require_once( get_template_directory() . '/widgets/' . 'property-travel-widget.php');
    require_once( get_template_directory() . '/widgets/' . 'property-market-info-widget.php');
    ...
    if(!function_exists('load_theme_styles')){
        function load_theme_styles(){
            if (!is_admin()){
                ...
                wp_register_style('bootstrap-slider-css',  get_template_directory_uri() . '/css/bootstrap-slider.css', array(), '2.2.2', 'all');
    ...
    if(!function_exists('load_theme_scripts')){
        function load_theme_scripts(){
            if (!is_admin()) {
                ...
                // Registering Scripts
                wp_register_script('jquery-ui', $java_script_url.'jquery-ui.js"', array('jquery'), '2.1', false);
                ...
                wp_register_script('bootstrap-slider', $java_script_url.'bootstrap-slider.js', array('jquery'), false);
                ...
                if( google_map_needed() ){
                    wp_register_script('google-map-api', '//maps.googleapis.com/maps/api/js?v=3.exp&signed_in=true&libraries=places,geometry', array(), '', false);
			...
			if($numbers_list_for == 'bedrooms')
			{
				foreach($numbers_array as $number){
					...
					if($searched_value == $number){
						echo '<option value="'.$number.'" selected="selected">'.$number.' '.$beds.'</option>';
					}else {
						echo '<option value="'.$number.'">'.$number.' '.$beds.'</option>';
					}
...
if(!function_exists('min_prices_list')){
    function min_prices_list(){
        ...
        $min_price_array = array( 100000, 200000, 300000, 400000, 500000, 600000, 700000, 800000, 900000, 1000000, 1500000, 2000000, 2500000, 5000000 );
...
/*-----------------------------------------------------------------------------------*/
// Display figcaption based on property statuses
/*-----------------------------------------------------------------------------------*/
    ...
    function display_figcaption( $post_id ){
        ...
        if( !empty( $status_terms ) ){
            ...
            if( !empty( $status_names) ){
                echo '<figcaption class="'.$status_classes.'">'.$status_names.'</figcaption>';
            }
        }
    }
}
...
if( !function_exists('inspiry_image_placeholder')){
    function inspiry_image_placeholder( $image_size ){
        ...
        if( intval($holder_width) > 0 && intval($holder_height) > 0 ) {
            echo '<img src="http://placehold.it/' . $holder_width . 'x' . $holder_height . '&text=' . urlencode($holder_text) . '" />';
        }
    }
}
...
function fpi_resize_at_upload( $file ) {
  ...
  // resize only if the mime type is image
	if ( $file['type'] == 'image/jpeg' OR $file['type'] == 'image/gif' OR $file['type'] == 'image/png') {
		...
		$w = intval(770 ); // get large size width
		$h = intval(386 ); // get large size height
